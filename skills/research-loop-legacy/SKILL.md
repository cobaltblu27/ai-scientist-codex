...
- Keep the project-local Stop hook installed and active for the main orchestrator session. Research workers are not loop owners; worker Stop should not be blocked when the active run is bound to a different orchestrator thread/session.
...
Scientist/researcher runs must freeze a `research_contract` in `config.json` before work begins. It must state the primary hypothesis, goal type, success criteria, failure criteria, allowed rescue scope, kill criteria, non-drift definition, metrics that matter, and non-negotiable comparisons. Performance-focused contracts must also include a usable baseline reference, benchmark plan, and target threshold. This contract is the standard critics use to distinguish supported hypotheses, solid negative results, and rescues from quiet claim narrowing.
...
Use native Codex subagents only for bounded, auditable work. Subagents are workers, not loop owners. The main orchestrator owns state transitions and final decisions. If worker launches can set environment variables, launch them with `AI_SCIENTIST_WORKER=1` so their Stop hook exits do not consume the orchestrator continuation gate.
