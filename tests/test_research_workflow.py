...
class ResearchWorkflowTests(unittest.TestCase):
    def test_skill_and_prompt_layout_exists(self) -> None:
        ...
        self.assertIn("Approach-change opportunities", data_insight_revision)
        self.assertIn("Data Insight Work", data_insight_revision)
        self.assertIn("natural-language coordination hints, not hard locks", data_insight_revision)
        for mode in ["scientist", "engineer", "custom"]:
            ...
            self.assertIn("If a substantially similar insight is already in progress", revision_text)
    ...
    def test_campaign_start_freezes_idea_batch_and_learning_notes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(cfg["discovery_notes_ref"].endswith("/discovery-notes.md"))
            ...
            self.assertEqual(state["state"]["discovery_notes"]["path"], cfg["discovery_notes_ref"])
            self.assertEqual(state["state"]["discovery_notes"]["status"], "active")
            discovery_ref = Path(cfg["discovery_notes_ref"])
            self.assertTrue(discovery_ref.exists())
            discovery_text = discovery_ref.read_text()
            self.assertIn("# Discovery Notes", discovery_text)
            self.assertIn("## Current Best Understanding", discovery_text)
            self.assertIn("## Node Notes", discovery_text)
