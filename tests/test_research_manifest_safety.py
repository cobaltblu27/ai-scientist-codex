...
from research.loop import manifests
...
def call_manifest_sanitizer(manifest: dict, workspace: Path):
    for name in ("sanitize_manifest", "validate_manifest", "validate_and_materialize_manifest", "materialize_manifest"):
        func = getattr(manifests, name, None)
        if func is None:
            continue
        errors = []
        for args, kwargs in (
    ...
    raise AssertionError("research.loop.manifests must expose a manifest sanitizer/materializer function")
