...
from research.loop import dispatcher as dispatcher_module
from research.loop import orchestrator
...
class DispatcherTests(unittest.TestCase):
    def test_dispatcher_starts_nodes_fifo_and_respects_parallel_resource_caps(self) -> None:
        ...
        if hasattr(dispatcher_module, "Dispatcher"):
            dispatcher = dispatcher_module.Dispatcher(max_parallel=1, max_gpus=1, max_cpu_cores=2, max_memory_mb=1024)
            ...
            self.assertTrue(hasattr(module, "FifoDispatcher"), "research.loop.dispatcher must expose Dispatcher or FifoDispatcher")
            self.assertTrue(hasattr(module, "ResourceLimiter"), "research.loop.dispatcher must expose ResourceLimiter")
