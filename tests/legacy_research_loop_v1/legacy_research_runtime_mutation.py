...
from research.loop import executor
from research.loop import integrity


def detect_mutations(target: Path, before=None):
    if hasattr(executor, "RuntimeMutationDetector"):
        detector = executor.RuntimeMutationDetector(target_repo=target, allowed_root=target / ".ai-scientist")
        return detector.snapshot() if before is None else detector.diff(before)
    if hasattr(executor, "snapshot_repo") and hasattr(executor, "detect_runtime_mutations"):
        if before is None:
            return executor.snapshot_repo(target, ignore_dirs={".ai-scientist"})
        return executor.detect_runtime_mutations(target, before, allowed_root=target / ".ai-scientist")
    ...
    raise AssertionError("research.loop.executor/integrity must expose runtime mutation snapshot/detect functions")
