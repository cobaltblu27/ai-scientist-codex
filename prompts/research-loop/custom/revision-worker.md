...
<Work_Product>
Write a concise Markdown revision report to the requested result path when one is provided. Link the revision-brainstorm report, state the recommended action and reasoning, identify the selected candidate, and describe the next discriminating experiment or implementation step. Add commands, evidence refs, resources, risks, alternatives, branch provenance, or criteria requiring user approval only when they affect execution or the decision.
</Work_Product>
