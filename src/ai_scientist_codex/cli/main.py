...
from ai_scientist_codex.core.state import (
...
from ai_scientist_codex.writeup.state import (
...
from ai_scientist_codex.research.usage_cap import (
    UsageCapError,
    is_snapshot_fresh,
...
def cmd_validate_run(args: argparse.Namespace) -> int:
    from ai_scientist_codex.validation.run import main as validate_run_main

    argv = [str(args.target), "--gate", args.gate, "--validation-mode", args.validation_mode]
    if args.run_id:
        argv.extend(["--run-id", args.run_id])
    return validate_run_main(argv)


def cmd_validate_idea(args: argparse.Namespace) -> int:
    from ai_scientist_codex.ideation.validate import main as validate_idea_main

    argv = ["--idea-json", str(args.idea_json)]
    for path in args.search_cache:
        argv.extend(["--search-cache", str(path)])
    return validate_idea_main(argv)


def cmd_hooks_install(args: argparse.Namespace) -> int:
    from ai_scientist_codex.hooks.install import main as install_main
...
def cmd_hooks_codex_event(args: argparse.Namespace) -> int:
    from ai_scientist_codex.ideation.hooks import main as hooks_main

    return hooks_main([args.event])


def cmd_hooks_stop_gate(args: argparse.Namespace) -> int:
    from ai_scientist_codex.hooks.stop_gate import main as stop_gate_main
...
def cmd_evidence_semantic_scholar(args: argparse.Namespace) -> int:
    from ai_scientist_codex.ideation.evidence import main as evidence_main
...
def cmd_research_loop_run(args: argparse.Namespace) -> int:
    from ai_scientist_codex.research.loop.orchestrator import main as research_loop_main
...
def build_parser() -> argparse.ArgumentParser:
    ...
    validate_run.add_argument("--gate", choices=["ideation_to_research", "research_to_review", "review_to_writeup", "launch", "principles", "all"], default="all")
    validate_run.add_argument("--run-id")
    validate_run.add_argument("--validation-mode", choices=["evidence", "final"], default="evidence")
    validate_run.set_defaults(func=cmd_validate_run)
    validate_idea = validate_sub.add_parser("idea")
    validate_idea.add_argument("--idea-json", type=Path, required=True)
    validate_idea.add_argument("--search-cache", type=Path, action="append", default=[])
    validate_idea.set_defaults(func=cmd_validate_idea)
