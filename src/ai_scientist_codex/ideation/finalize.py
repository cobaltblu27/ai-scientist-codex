...
from ai_scientist_codex.ideation.state import ai_dir, append_journal, diff_snapshot, read_json, run_dir, save_state, utc_now, write_json
...
def run_validator(plugin_root: Path, target_repo: Path, run_id: str) -> int:
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "ai_scientist_codex.validation.run",
            str(target_repo),
            "--gate",
