...
PLUGIN_ROOT = REPO_ROOT / "plugins" / "ai-scientist"
SCRIPTS_DIR = PLUGIN_ROOT / "scripts"
RESEARCH_ORCHESTRATOR = SCRIPTS_DIR / "research_orchestrator.py"
VALIDATE_RUN = SCRIPTS_DIR / "validate_run.py"

if str(SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_DIR))


def import_planned_module(name: str):
    try:
        return importlib.import_module(name)
    except ModuleNotFoundError as exc:
        raise AssertionError(f"planned module {name!r} is not importable yet: {exc}")
...
def write_minimal_research_run(target: Path, *, decision: str | None = "approved", direction: str = "maximize") -> Path:
    ...
    snapshot = import_planned_module("research_loop.handoff").artifact_snapshot(run)
    ...
    if decision is not None:
        write_json(run / "verifier-decisions" / "research_to_review.json", {"decision": decision, "gate": "research_to_review", "artifact_snapshot": snapshot, "validation_command": "validate_run.py --gate research_to_review --validation-mode evidence"})
    return run
