...
def build_prompt(action: str, mode: str, node_dir: Path) -> dict:
    module = import_planned_module("research_loop.prompts")
    kwargs = {
        "action": action,
    ...
    if hasattr(module, "build_prompt"):
        try:
            result = module.build_prompt(**kwargs)
        except TypeError:
            ...
            result = module.build_prompt(config, action, f"{action}-{mode}", kwargs["parent_node_id"], kwargs["idea"])
    elif hasattr(module, "PromptBuilder"):
        result = module.PromptBuilder().build(**kwargs)
    else:
        raise AssertionError("research_loop.prompts must expose build_prompt or PromptBuilder.build")
