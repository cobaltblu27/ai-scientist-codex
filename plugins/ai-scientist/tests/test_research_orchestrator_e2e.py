...
from test_support import VALIDATE_RUN, make_idea_json, make_research_target, orchestrator_args, read_json, run_python
    ...
    def test_fixture_orchestrator_e2e_passes_final_validation_for_maximize_and_minimize(self) -> None:
        ...
        for metric_key, direction, threshold in cases:
            with self.subTest(direction=direction), TemporaryDirectory() as td:
                ...
                validation = run_python([VALIDATE_RUN, target, "--gate", "research_to_review", "--run-id", run_id, "--validation-mode", "final"])
