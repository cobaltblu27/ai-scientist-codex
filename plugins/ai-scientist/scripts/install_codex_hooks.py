...
from ai_scientist_state import start_phase
...
def is_ai_scientist_hook_entry(entry: Any) -> bool:
    ...
    for hook in hooks:
        ...
        if isinstance(command, str) and "ai_scientist_stop_hook.py" in command:
            return True
    return False


def install_hooks_json(project_root: Path, python: str, script: Path) -> Path:
    ...
    stop_entries.append(
        {
            "hooks": [
                {
                    "type": "command",
                    "command": hook_command(python, script),
                    "timeout": 30,
                }
            ]
        }
...
def main() -> int:
    ...
    script = Path(__file__).resolve().with_name("ai_scientist_stop_hook.py")
    ...
    hooks_path = install_hooks_json(project_root, args.python, script)
